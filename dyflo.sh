...
#
#   dyflo.sh                 interactive menu (assign vs do-it-yourself)
#   dyflo.sh --self          do the work yourself (equipped interactive session)
#   dyflo.sh --assign        route all open tickets into their lanes
...
do_assign() {
  ...
  echo "  • HITL tickets  → dyflo.sh --assign <id>   (runs the research stage)"
...
case "${1:-}" in
  ...
  *) echo "usage: dyflo.sh [--bootstrap|--self|--assign [id]|--docs [focus]|--check]"; exit 2 ;;
esac
